...
CH <- read.csv("CMR_Bjornoya1.csv", header = FALSE)
BS <- read.csv("BS_Bjornoya.csv")
Counts <- read.csv("Counts_Bjornoya.csv")
...
yr.cmr <- 2004:(2004 + n.occasions - 1)
yr.bs <- BS$Year
...
cat("--- Test run ---\n")
test <- jags(jags.data, inits, parameters, "IPM_bjornoya.txt",
...
out <- jags(jags.data, inits, parameters, "IPM_bjornoya.txt",
...
sink("../output/IPM_Bjornoya_out.txt")
print(out, digits = 3)
sink()
save(out, file = "../output/IPM_Bjornoya.Rdata")
