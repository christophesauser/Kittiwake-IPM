...
CH <- read.csv("CMR_May.csv", header = FALSE)
BS <- read.csv("BS_May.csv")
Counts <- read.csv("Counts_May.csv")
...
yr.cmr <- 1986:(1986 + n.occasions - 1)
yr.bs <- BS$Year
...
cat("--- Test run ---\n")
test <- jags(jags.data, inits, parameters, "IPM_may.txt",
...
ni <- 200000; nb <- 50000; nc <- 3; nt <- 5; na <- 5000

out <- jags(jags.data, inits, parameters, "IPM_may.txt",
...
sink("IPM_May_out.txt")
print(out, digits = 3)
sink()
save(out, file = "IPM_May.Rdata")
