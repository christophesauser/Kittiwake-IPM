...
CH <- read.csv("CMR_Hjelmsoya.csv", header = FALSE)
BS <- read.csv("BS_Hjelmsoya.csv")
Counts <- read.csv("Counts_Hjelmsoya.csv")
...
yr.cmr <- 2001:(2001 + n.occasions - 1)
yr.bs <- BS$Year
...
cat("--- Test run ---\n")
test <- jags(jags.data, inits, parameters, "IPM_hjelmsoya.txt",
...
out <- jags(jags.data, inits, parameters, "IPM_hjelmsoya.txt",
...
sink("../output/IPM_Hjelmsoya_out.txt")
print(out, digits = 3)
sink()
save(out, file = "../output/IPM_Hjelmsoya.Rdata")
