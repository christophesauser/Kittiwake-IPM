...
CH <- read.csv("CMR_Runde.csv", header = FALSE)
BS <- read.csv("BS_Runde.csv")
BS <- BS[BS$AON > 1, ]
Counts <- read.csv("Counts_Runde.csv")
...
cat("--- Test run ---\n")
test <- jags(jags.data, inits, parameters, "IPM_runde.txt",
...
out <- jags(jags.data, inits, parameters, "IPM_runde.txt",
...
sink("../output/IPM_Runde_out.txt")
print(out, digits = 3)
sink()
save(out, file = "../output/IPM_Runde.Rdata")
