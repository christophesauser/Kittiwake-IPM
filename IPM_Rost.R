...
CH <- read.csv("CMR_Rost.csv", header = FALSE)
BS <- read.csv("BS_Rost.csv")
Counts <- read.csv("Counts_Rost.csv")
...
yr.cmr <- 2003:(2003 + n.occasions - 1)
yr.bs <- BS$Year
...
cat("--- Test run ---\n")
test <- jags(jags.data, inits, parameters, "IPM_rost.txt",
...
out <- jags(jags.data, inits, parameters, "IPM_rost.txt",
...
sink("../output/IPM_Rost_out.txt")
print(out, digits = 3)
sink()
save(out, file = "../output/IPM_Rost.Rdata")
