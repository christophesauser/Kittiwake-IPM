...
CH <- read.csv("CMR_Hornoya.csv", header = FALSE)
BS <- read.csv("BS_Hornoya.csv")
Counts <- read.csv("Counts_Hornoya.csv")
...
yr.cmr <- 1990:(1990 + n.occasions - 1)
yr.bs <- BS$Year
...
# TEST RUN (~2 min, checks compilation and basic sampling)

cat("--- Test run ---\n")
test <- jags(jags.data, inits, parameters, "IPM_hornoya.txt",
...
out <- jags(jags.data, inits, parameters, "IPM_hornoya.txt",
...
sink("../output/IPM_Hornoya_out.txt")
print(out, digits = 3)
sink()
save(out, file = "../output/IPM_Hornoya.Rdata")